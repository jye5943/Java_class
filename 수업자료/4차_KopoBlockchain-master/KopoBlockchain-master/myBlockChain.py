...
def generateGenesisBlock():
    ...
    timestamp = time.time()
    print("time.time() => %f \n" % timestamp)
    tempHash = calculateHash(0, '0', timestamp, "Genesis Block", 0)   #genesis block은 원래 거래량이 들어가야 되는데 없어서 넣은 것
    print(tempHash)
...
def writeBlockchain(blockchain):  #blockchain은 리스트임 (아래서 def있음)
    blockchainList = []     #blcokchainList라는 리스트를 선언 [] 이런 껍데기인거!
                            #blockchainList에 blockchain을 넣으면 이중배열인거! 예를 들어 (write 할)블록이 두 개라면 .... [ [o,o,o,],[o,o,o]]형태인거
                            #그렇다면 왜 풀었다가 다시 묶는 귀찮은 작업을 또 하는건가? = > 교수님께서 얘기하셨는데 잘 못들음 ;

    for block in blockchain:    #블록을 포문 돌리는데 어차피 한 번만 돌 것!
    ...
    with open(g_bcFileName, "w", newline='') as file:
        ...
        writer.writerows(blockchainList) # writer.writerows : 여러 행을 write 한다는 뜻

    # update txData cause it has been mined.
    for block in blockchain:            # 처음 도는 제네시스 블록의 생성.
        updateTx(block)
...
def readBlockchain(blockchainFilePath, mode = 'internal'):
    ...
    try:
        ...
        if mode == 'internal' :
            blockchain = generateGenesisBlock()  #blockchain에 genesis블록의 객치를 넣은 것 (list안에 객체가 들어간 것)
